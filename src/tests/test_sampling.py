...
def test_generate_samples():
    ...
    assert len(X_sobol) == 128*(2*problem['num_vars']+2), f"Unexpected number of samples for Sobol {len(X_sobol)}"
...
def test_generate_samples_with_model():
    ...
    assert len(report.x_lhs) == 128*problem['num_vars'], f"Unexpected number of samples for LHS {len(report.x_lhs)}"
    assert len(report.x_morris) == 128*(problem['num_vars']+1), f"Unexpected number of samples for Morris {len(report.x_morris)}"
    assert len(report.x_sobol) == 128*(2*problem['num_vars']+2), f"Unexpected number of samples for Sobol {len(report.x_sobol)} != {128*(2*problem['num_vars']+2)}"

...
deltaftarget = 1e-8
nb_evaluations_always_written = (
    "1"  # '100 + 10 * dim'  # 100 + dim;10*dim add about 7;17MB to final data
)
...
class LoggingFunction(object):
    ...
    * data files (:file:`dat`, :file:`tdat` extensions) which record
    ...
    nbptsevals = 20.0  # number of trigger per decade of function evaluations
    nbptsf = float(
        nb_triggers_per_delta_f_decade
    )  # number of trigger per decade of function values, float to prevent integer division
    ...
    def __init__(self, datapath, algid="not-specified", comments="", inputformat="row"):
    ...
    def initialize(
        self, datapath, algid="not-specified", comments="", inputformat="row"
    ):
    ...
    def _readytostart(self):
        ...
        filename = "%s_f%s_DIM%d" % (self.datafileprefix, str(self.funId), self._dim)

        res = []
        if not (self._is_samefun and self._is_samedim) or not os.path.exists(
            self.indexfile
        ):
            i = 0
            while os.path.exists(filename + ".dat"):
                i += 1
                filename = "%s-%02d_f%s_DIM%d" % (
                    self.datafileprefix,
                    i,
                    str(self.funId),
                    self._dim,
            ...
            res.append(
                ", DIM = %d, Precision = %.3e, algId = '%s'\n"
                % (self._dim, self.precision, self.algid)
            )
            res.append(
                "%% %s\n%s"
                % (self.comments, os.path.relpath(self.hdatafile, self.datapath))
            )
        ...
        for datafile in (self.datafile, self.hdatafile, self.rdatafile):
            ...
            f = open(datafile, "a")
            f.write(
                "%% function evaluation | noise-free fitness - Fopt"
    ...
    def evalfun(self, inputx, *args, **kwargs):
        ...
        if (
            self.lasteval.num + popsi >= self.evalsTrigger
            or np.min(ftrue) - self.fopt < self.fTrigger
            ...
            for j in range(0, popsi):
                ...
                if self.lasteval.num >= self.evalsTrigger:
                    ...
                    while self.lasteval.num >= np.floor(
                ...
                if ftruej - self.fopt < self.fTrigger:  # minimization
                    ...
                    if ftruej <= self.fopt:
                        ...
                        if np.isinf(self.idxFTrigger):
                            self.idxFTrigger = (
                                np.ceil(np.log10(ftruej - self.fopt)) * self.nbptsf
                            )
                        while ftruej - self.fopt <= 10 ** (
                            self.idxFTrigger / self.nbptsf
                        ):
                            self.idxFTrigger -= 1
                        self.fTrigger = min(
                            self.fTrigger, 10 ** (self.idxFTrigger / self.nbptsf)
                        )  # TODO: why?
            # write
            if buffr:
    ...
    def finalizerun(self):
        ...
        f.write(
            ":%d|%.1e"
            % (self.lasteval.num, self.lasteval.bestf - self.fopt - self.precision)
        )
        f.close()
        self._is_finalized = True
    ...
    def _setdatapath(self, datapath):
        try:
            ...
            (err, strerror) = xxx_todo_changeme1.args
            if err in (errno.EEXIST, 183):  # 183 it is on Windows 7
                pass
            else:
                print(errno, strerror)
    ...
    def _setinputformat(self, inputformat):
        if inputformat in ("row", "col"):
            ...
            warnings.warn(
                "The inputFormat input argument is expected to"
    ...
    def _getevaluations(self):
        ...
        if not hasattr(self, "lasteval") or not self._is_setdim:
            ...
            return 0.0  # should be synchronized with what is assigned to __Eval.num
        else:
    ...
    def setfun(
        ...
        **kwargs
        ...
        self.datafileprefix = os.path.join(
            self.datapath, "data_f%s" % str(self.funId), self.fileprefix
        )
        self.indexfile = os.path.join(
            self.datapath, "%s_f%s.info" % (self.fileprefix, str(self.funId))
        )
    ...
    def _setdim(self, dim):
        ...
        self.idxEvalsTrigger = 0.0
        self.evalsTrigger = 1.0
        self.nbFirstEvalsToAlwaysWrite = eval(
            nb_evaluations_always_written.replace("dim", str(dim))
        )  # TODO:
        self.idxDIMEvalsTrigger = 0.0
        self.fTrigger = np.inf
